...
if (is.na(params$bayes$model[1])) {bayes_text <- "Bayesian analysis was not conducted in the MetaInsight app"
  } else {
    bayes_text <- ""
...
if (is.na(params$bayes$model[1])) {model_fit_text <- ""
} else {
...
if (is.na(params$bayes$model_sub[1])) {bayes_text_sub <- "Bayesian subgroup analysis was not conducted in the MetaInsight app"
} else {
    bayes_text_sub <- ""
...
if (is.na(params$bayes$model_sub[1])) {model_sub_text <- ""
} else {
...
```{r echo = FALSE}
if (!is.na(params$bayes$model[1])) {kableExtra::kable(baye_comp(params$bayes$model, params$metaoutcome, params$outcome_measure))}
```
...
if (!is.na(params$bayes$model_sub[1])) {kableExtra::kable(baye_comp(params$bayes$model_sub, params$metaoutcome, params$outcome_measure))}
```
...
```{r echo = FALSE}
if (is.na(params$model_nodesplit_sub)) {node_sub_text <- "The nodesplit subgroup model was not run in the MetaInsight app"
...
```{r echo = FALSE}
if (!is.na(params$bayes$model[1])) {params$bayes$model$sumresults}
```
...
```{r echo = FALSE}
if (!is.na(params$bayes$model[1])) {gelman.plot(params$bayes$model$mtcResults)}
```
...
```{r echo = FALSE}
if (!is.na(params$bayes$model_sub[1])) {params$bayes$model_sub$sumresults}
```
...
```{r echo = FALSE}
if (!is.na(params$bayes$model_sub[1])) {gelman.plot(params$bayes$model_sub$mtcResults)}
```
...
if (!is.na(params$bayes$model[1])) {scat_plot(params$bayes$model)$p}
```
...
```{r echo = FALSE, warning = FALSE, message = FALSE}
if (!is.na(params$bayes$model_sub[1])) {scat_plot(params$bayes$model_sub)$p}
```
...
if (!is.na(params$bayes$model[1])) {stemplot(params$bayes$model)}
```
...
```{r echo = FALSE, message = FALSE}
if (!is.na(params$bayes$model_sub[1])) {stemplot(params$bayes$model_sub)}
```
...
```{r echo = FALSE, message = FALSE}
if (!is.na(params$bayes$model[1])) {levplot(params$bayes$model)}
```
...
```{r echo = FALSE, message = FALSE}
if (!is.na(params$bayes$model_sub[1])) {levplot(params$bayes$model_sub)}
```
...
```{r echo = FALSE}
if (is.na(params$bayes$model[1])) {code_text <- bayes_text
} else {code_text <- params$bayes$model$mtcResults$model$code}
```
...
```{r echo = FALSE}
if (!is.na(params$bayes$model[1])) {params$bayes$model$mtcResults$model$inits}
```
...
This tab has been deliberately left empty as the associated MetaInsight tab contains only download buttons
...
```{r echo = FALSE}
if (!is.na(params$bayes$model[1])) {mtc.deviance({params$bayes$model$mtcResults})}
```
...
```{r echo = FALSE, warning = FALSE}
if (!is.na(params$bayes$model[1])) {scat_plot(params$bayes$model)$y}
```
...
```{r echo = FALSE}
if (!is.na(params$bayes$model_sub[1])) {mtc.deviance({params$bayes$model_sub$mtcResults})}
```
...
```{r echo = FALSE, warning = FALSE}
if (!is.na(params$bayes$model_sub[1])) {scat_plot(params$bayes$model_sub)$y}
```
